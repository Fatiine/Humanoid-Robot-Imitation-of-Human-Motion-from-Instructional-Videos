...
def smpl_to_deepmimic(poses, trans, cams): #, proc_param):
    ...
    #frames = np.zeros((num_img + 1, 44))# PROVISOIRE !!
    for i in range(num_img):
        frames[i] = kinematic_tree(poses[i], trans[i], cams[i])
        #frames[i+1] = kinematic_tree(poses[i], trans[i], cams[i])  # PROVISOIRE !!! # proc_param[i])
    origin = frames[0, 1:4]
    frames[:, 1:4] = frames[:, 1:4] - origin + np.array([0, 0.89, 0])
    return frames
